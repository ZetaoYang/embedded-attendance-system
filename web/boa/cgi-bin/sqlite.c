...
int sqliteDB_insert_records()
{
 	...
 	//char sql1[256]="INSERT INTO persons VALUES('syd','140410408','140410408');";
...
//	printf("\n rc = %d",rc);
	if( rc!=SQLITE_OK ){
		fprintf(stderr, "SQL error: %s\n", zErrMsg);
		printf("<h1>添加员工失败,员工号重复</h1>",zErrMsg);
		//printf("\n########add people fail %s\n#######################",zErrMsg);
		sqlite3_free(zErrMsg);
		return false;
	}else{
...
int sqliteDB_opt_Sign_Work(char* pnumber,int date,int cometime,int isLate){
	...
	char *zErrMsg = 0;
	//char *sql=0;//动态生成的SQL语句
	char sql[256]="insert into routines(pnumber,date,cometime,isLate,leavetime,isEarlyLeave) values(140410408,20170707,111111,1,111111,1);";
